...
export async function selectApprovedAnswer(question, approved, options = {}) {
  ...
  try {
    const response = await fetcher(ENDPOINT, {
      ...
      body: JSON.stringify({ messages: [{ role: 'system', content: selectionPrompt(approved) }, { role: 'user', content: question }], max_tokens: 128, temperature: 0, stream: false, response_format: { type: 'json_object' } })
    });
